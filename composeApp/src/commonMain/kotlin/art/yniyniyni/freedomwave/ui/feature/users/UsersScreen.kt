    ...
    when (val current = nav) {
        is UsersNav.List ->
            Scaffold(
                ...
                floatingActionButton = {
                    FloatingActionButton(onClick = { /* TODO Phase 3: create user */ }) {
                            ...
                            LazyColumn(contentPadding = PaddingValues(16.dp), verticalArrangement = Arrangement.spacedBy(8.dp)) {
                                items(state.filtered, key = { it.uuid }) { user ->
                                    UserListItem(
                                        ...
                                        onToggle = { if (user.isActive) vm.disableUser(user.uuid) else vm.enableUser(user.uuid) },
                                        onReset = { vm.resetTraffic(user.uuid) }
                                    )
                                }
                            }
                    }
        ...
        is UsersNav.Detail ->
            UserDetailScreen(
                ...
                onEnable = { vm.enableUser(current.user.uuid); nav = UsersNav.List },
                onDisable = { vm.disableUser(current.user.uuid); nav = UsersNav.List },
    ...
    Card(modifier = Modifier.fillMaxWidth().clickable(onClick = onClick)) {
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    formatBytes(user.usedTrafficBytes) + if (user.trafficLimitBytes > 0) " / ${formatBytes(user.trafficLimitBytes)}" else " / ∞",
        ...
        LazyColumn(
            ...
            item {
                Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
                    ...
                    Button(onClick = onResetTraffic, modifier = Modifier.fillMaxWidth(),
